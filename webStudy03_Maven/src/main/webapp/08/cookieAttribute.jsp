...
<body>
	<h4>쿠키의 속성들</h4>
	<pre>
		1. path : 브라우저의 쿠키 저장소에 저장된 쿠키가 다시 서버로 재전송 될 지 여부를 결정,
				  쿠키에 설정된 path 속성의값을 확인하여,
				  해당 경로의 하위 경로로 발생하는 요청에만 쿠키가 재전송
				  생략한 경우, 쿠키의 생성 경로를 기본값으로 사용함
		2. domain/host : 저장된 쿠키의 재전송 여부를 결정할때 사용
						생략된경우, 쿠키가 생성된 도메인(호스트)가 사용됨
						특정 사이트가 보유한 모든 호스트에 전송될 쿠키를 생성할때 사용.
						도메인의 레벨 구조에 다라 ".naver.com" 방식으로 설정함
...
// 			otherDomainCookie.setDomain(".gay.com");
...
// 			response.addCookie(otherDomainCookie);
		...
		%>
		<a href="cookieView.jsp">쿠키 확인하기(동일경로에서)</a>
		<a href="08inner/cookieView.jsp">쿠키 확인하기(하위경로에서)</a>
		<a href="<%=request.getContextPath() %>/09/cookieView.jsp">쿠키 확인하기(다른경로에서)</a>
	</pre>
</body>
</html>