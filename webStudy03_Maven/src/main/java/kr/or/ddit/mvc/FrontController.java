...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		for(String uri : keySet) {
			...
			try {
				Class<ICommandHandler> handlerClz = (Class<ICommandHandler>) Class.forName(qualifiedName);
				ICommandHandler handler = handlerClz.newInstance(); // 리플랙션 ?
...
//		1. 요청 매핑 설정 (xml에 완료)
		...
		String uri = req.getRequestURI();  // 요청분석을 위한 주소를 받음
		///member/memList.do
...
//		8. 뷰로 이동
		...
		String suffix = ".jsp";
		if(view!=null) {
			boolean redirect = view.startsWith("redirect:"); // redurect : 로 시작 한다면 트루값
			if (redirect) {
				// 트루라면 일루 
			...
			if(!resp.isCommitted()) {
				resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
						"커맨드 핸들러에서 뷰가 선택되지 않았습니다."); // 500 에러
			}
		}
	}
}
